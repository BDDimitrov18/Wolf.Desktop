...
public class TaskTypePaymentInquiry
{
    private readonly List<RequestDto> _requests;
    private readonly List<EmployeeDto> _employees;
    private List<TasktypeDto> _allTaskTypes = [];

    public TaskTypePaymentInquiry(List<RequestDto> requests, List<EmployeeDto> employees)
    {
        _requests = requests;
            ...
            foreach (var employee in _employees)
            {
                ...
                var employeeRequests = _requests.Where(req =>
                {
                    ...
                    return activities.Any(act =>
                        cache.GetTasksForActivity(act.Activityid).Any(t => t.Executantid == employee.Employeeid));
                }).ToList();

                int employeeRowSpan = 0;
                uint employeeStartRow = rowIndex;

                foreach (var request in employeeRequests)
                {
                    var employeeActivities = cache.GetActivitiesForRequest(request.Requestid)
                        .Where(act => cache.GetTasksForActivity(act.Activityid)
                            .Any(t => t.Executantid == employee.Employeeid))
                        .ToList();

                    int requestRowSpan = 0;
                    uint requestStartRow = rowIndex;
                    ...
                    foreach (var activity in employeeActivities)
                    {
                        ...
                        var employeeTasks = cache.GetTasksForActivity(activity.Activityid)
                            .Where(t => t.Executantid == employee.Employeeid)
                            .ToList();

                        int colIdx = 7;
                        foreach (var taskType in _allTaskTypes)
                        {
                            var sum = employeeTasks.Where(t => t.Tasktypeid == taskType.Tasktypeid)
                                .Sum(t => t.Executantpayment);
                            AddNumberCell(row, colIdx, sum.ToString(), wrapStyle);
                            colIdx++;
                        }
                    ...
                    if (requestRowSpan > 1)
                    {
                        MergeCells(worksheetPart, requestStartRow, 2, rowIndex - 1);
                        MergeCells(worksheetPart, requestStartRow, 3, rowIndex - 1);
                        MergeCells(worksheetPart, requestStartRow, 4, rowIndex - 1);
                    }
                }

                if (employeeRowSpan > 1)
                    MergeCells(worksheetPart, employeeStartRow, 1, rowIndex - 1);
            }

            AddTaskTypeLegend(sheetData, rowIndex, wrapStyle);
    ...
    private void CollectAllTaskTypes(DataCacheService cache)
    {
        ...
        foreach (var request in _requests)
        {
            ...
            foreach (var act in activities)
            {
                ...
                {
                    if (typeIds.Add(task.Tasktypeid))
                    {
                        var tt = cache.GetTaskType(task.Tasktypeid);
                        if (tt is not null) types.Add(tt);
                    }
                }
            }
        }

        _allTaskTypes = types.OrderBy(t => t.Tasktypename).ToList();
    }
    ...
    private void AddHeaderRow(SheetData sheetData)
    {
        ...
        int colIdx = 7;
        int key = 1;
        foreach (var _ in _allTaskTypes)
        {
            AddTextCell(row, colIdx, key.ToString());
            colIdx++;
            key++;
        }
    }

    private void AddTaskTypeLegend(SheetData sheetData, uint currentRow, uint styleIndex)
    {
        currentRow++;
        sheetData.Append(new Row { RowIndex = currentRow });

        currentRow++;
        var legendHeader = new Row { RowIndex = currentRow };
        sheetData.Append(legendHeader);
        AddTextCell(legendHeader, 1, "Легенда:", styleIndex);

        int key = 1;
        foreach (var taskType in _allTaskTypes)
        {
            currentRow++;
            var legendRow = new Row { RowIndex = currentRow };
            sheetData.Append(legendRow);
            AddTextCell(legendRow, 1, $"{key} - {taskType.Tasktypename}", styleIndex);
            key++;
        }
    }
    ...
    private static void SetColumnWidths(WorksheetPart wsp)
    {
        ...
        cols.Append(new Column { Min = 3, Max = 3, Width = 15, CustomWidth = true });
        cols.Append(new Column { Min = 4, Max = 4, Width = 25, CustomWidth = true });
        cols.Append(new Column { Min = 5, Max = 5, Width = 30, CustomWidth = true });
        cols.Append(new Column { Min = 6, Max = 6, Width = 20, CustomWidth = true });
        // Dynamic task type columns will use default width
